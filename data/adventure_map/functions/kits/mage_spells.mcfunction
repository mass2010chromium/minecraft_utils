...
execute at @a[tag=mage_splash] run scoreboard players operation @e[distance=0..3,type=!minecraft:player,scores={fine_hp.hp=1..},limit=3,sort=nearest] fine_hp.mdmg += @p spell.1.power
...
execute as @e[tag=replenish_1,nbt={Item:{tag:{display:{Name:'"Fireball 1"'}}}},scores={replenish_ok=1}] run execute at @s run execute at @p[tag=mage,tag=!replenish_fail,distance=0..4,scores={mana.mana=200..},nbt={Inventory:[{Slot:1b,tag:{display:{Name:'"Fireball 1"'}}}]}] store success score @p[tag=mage,tag=!replenish_fail,distance=0..4,scores={mana.mana=200..},nbt={Inventory:[{Slot:1b,tag:{display:{Name:'"Fireball 1"'}}}]}] fine_hp.tmp0 run tag @s add fireball_summon
