...
scoreboard players set @a[tag=pa_1] spell.2.power 10
# Different tiers
...
execute as @a[tag=shotPierce] at @s run scoreboard players operation @e[type=minecraft:spectral_arrow,tag=!processed,limit=1,sort=nearest] spell.2.power = @s spell.2.power 
# Different tiers
...
execute as @e[type=minecraft:spectral_arrow] at @s run scoreboard players operation @e[team=Enemies,distance=0..3,nbt={ActiveEffects:[{Id:24b}]}] fine_hp.mdmg += @s spell.2.power
