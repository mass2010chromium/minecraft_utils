...
execute at @p[tag=mage,nbt={SelectedItemSlot:2},tag=!replenish_fail,distance=0..4,scores={mana.mana=500..}] store success score @p[tag=mage,nbt={SelectedItemSlot:2},tag=!replenish_fail,distance=0..4,scores={mana.mana=500..}] fine_hp.tmp0 run tag @s add discharge_cast
...
execute at @s[tag=discharge_cast] run effect give @e[team=Enemies,distance=0..6] minecraft:slowness 3 3
execute at @s[tag=discharge_cast] at @e[team=Enemies,distance=0..6] run particle minecraft:large_smoke ~ ~1 ~ 0.25 1 0.25 0 50
execute at @s[tag=discharge_cast] at @e[team=Enemies,distance=0..6] run particle minecraft:explosion ~ ~1 ~
